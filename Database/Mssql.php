...
class Mssql extends DataBase
{
     ...
     * @var string
     */
    private $selectSql = 'SELECT%LIMIT%%DISTINCT% %FIELD% FROM %SCHEMAS%.%TABLE% %ALIA% %FORCE%%JOIN%%WHERE%%GROUP%%HAVING%%ORDER%%OFFSET%%UNION%%LOCK%%COMMENT%';
    ...
    {
        if (!$this->dsn) {
            $this->dsn = "sqlsrv:Server={$this->settings['host']},{$this->settings['port']};Database={$this->settings['dbname']}";
        }
        return $this->dsn;
    }
    ...
    private function getFieldType($table = null)
    {
        ...
        if (empty($this->_tempFieldType[$table])) {
            ...
            foreach ($result as $v) {
                if ($alia && $originTable) {
                    $ft[$originTable . '_' . $v['COLUMN_NAME']] = $v['TYPE_NAME'];
                } else {
