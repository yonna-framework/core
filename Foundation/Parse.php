...
namespace {
    ...
    class Parse{
         ...
         * @return array|string
         */
        public static function string($obj)
        {
            if (is_array($obj)) {
                foreach ($obj as $k => $v) {
                    if (is_array($v)) {
                        $obj[$k] = static::string($v);
                    } else {
         ...
         * @return array|string
         */
        public static function int($obj, $type = 'round')
        {
            if (is_array($obj)) {
                foreach ($obj as $k => $v) {
                    if (is_array($v)) {
         ...
         * @return array|string
         */
        public static function real($obj)
        {
            if (is_array($obj)) {
                foreach ($obj as $k => $v) {
                    if (is_array($v)) {
         ...
         * @return float|string
         */
        public static function scientificCountingMethod($obj)
        {
            if (is_array($obj)) {
                foreach ($obj as $k => $v) {
                    if (is_array($v)) {
                        $obj[$k] = static::scientificCountingMethod($v);
                    } else {
         ...
         * @return string
         */
        public static function name($name, $type = 0)
        {
            if ($type) {
