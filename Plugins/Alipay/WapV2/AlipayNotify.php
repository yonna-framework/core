...
class AlipayNotify extends AbstractLib{
	...
	function getSignVeryfy($para_temp, $sign) {
		...
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "RSA" :
				$isSign = $this->getRsa()->rsaVerify($prestr, trim($this->alipay_config['ali_public_key']), $sign);
				break;
			case "RSA2" :
				$isSign = $this->getRsa2()->rsaVerify($prestr, trim($this->alipay_config['ali_public_key']), $sign);
				break;
			default :
				$isSign = false;
		}
